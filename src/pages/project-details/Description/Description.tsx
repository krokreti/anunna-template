        ...
        >
            <Box display={'flex'} justifyContent={'end'} height={'100%'} flexDirection={'column'} component={"div"} position={'relative'}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} mt={2} p={0}>
                    <Typography variant="h6" fontWeight={'bold'} display={'flex'} alignItems={'center'}> <PlayArrowIcon color="primary" /> {t('details.project-genre')}</Typography>
                    <Typography variant="h6" ml={{ xs: 0, sm: 1 }} color={theme.palette.text.secondary} >{project.gender}</Typography>
                </Stack>
                <Stack marginY={4}>
                    <Typography variant="h4" fontWeight={'bold'} display={'flex'} alignItems={'center'} lineHeight={2.5}> {t('details.description')}</Typography>
                    {project.isActive && (
                ...
                <Stack direction={'row'} gap={2} mt={5}>
                    <CustomButton onClick={handleOpenDialog}
                        children={t('details.see-project')}
                        variant="outlined"
